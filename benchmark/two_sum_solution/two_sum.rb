...
def two_sum_performance_tests(size)
  arr = Array.new(size) { rand(size * 100) }

  Benchmark.bm(15) do |x|
    x.report("bad two sum:") {bad_two_sum?(arr, arr.last)}
    x.report("okay two sum a:") {okay_two_sum_a?(arr, arr.last)}
    x.report("okay two sum b:") {okay_two_sum_b?(arr, arr.last)}
    x.report("two sum:") {two_sum?(arr, arr.last)}
  end
end

two_sum_performance_tests(100000)
